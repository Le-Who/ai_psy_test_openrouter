...
NOTES: ${notes || "нет"}`;
  ...
  getTinyToken() {
    ...
    if (!token) {
      token = prompt("Введите ваш TinyURL API Token (из https://tinyurl.com/app/settings/api):");
      if (token) {
    ...
    async createShareLink() {
        ...
        if(!token) return alert("Нужен TinyURL Token для создания короткой ссылки!");
